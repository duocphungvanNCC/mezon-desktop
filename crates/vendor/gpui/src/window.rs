...
impl Drop for ElementArenaScope {
    fn drop(&mut self) {
        ...
        if !self.exited && !std::thread::panicking() {
            ...
            log::error!("ElementArenaScope dropped without calling exit()");
        }
    }
}
    ...
    pub(crate) fn new(
        ...
        platform_window.on_request_frame(Box::new({
            ...
            move |request_frame_options| {
                ...
                let high_rate_input = input_rate_tracker.borrow().is_high_rate();
        ...
        platform_window.on_appearance_changed(Box::new({
            ...
            move || {
                let mut cx = cx.clone();
                let handle = handle;
                // Zed #61789: AppKit can invoke this synchronously while App is borrowed.
                foreground_executor
                    .spawn(async move {
                        handle
...
impl Window {
    ...
    /// Pending input that can still complete a binding. Input left over from a
    /// previous focus can never complete one.
    ...
    /// Returns whether a GPUI-rendered prompt is currently active in this window.
...
mod frame_scheduling_tests {
    ...
    use crate::{AppContext as _, Empty, TestAppContext};
    ...
    fn queued_frame_callback_wakes_a_parked_render_loop(cx: &mut TestAppContext) {
        ...
        // Drain the initial render and its simulated compositor callback.
